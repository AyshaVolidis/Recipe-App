...
const createRecipesSection = () => {
  ...
  let title = dom.createElement(
    "h1",
    ["text-3xl", "font-bold", "text-red-800", "text-center", "pl-2", "mt-8", "mb-4"],
    "Recipes"
  );
  ...
  let toggleViewBtn = dom.createElement(
    "button",
    [
      "bg-red-700", "text-white", "font-medium", "px-4", "py-2",
      "rounded-md", "hover:bg-red-800"
    ],
    "View as List"
  );

  let inputWrapper = dom.createElement("div", ["relative", "w-full", "md:w-auto"]);
  let icon = dom.createElement("span", [
  ...
  let searchInput = dom.createElement(
    "input",
    [
      "border", "border-gray-400", "rounded-md", "pl-10", "py-2",
      "w-full", "md:w-[250px]", "focus:outline-red-700"
    ],
    "",
    {
      type: "text",
  ...
  dom.appendToParent(inputWrapper, [icon, searchInput]);

  let controlsRow = dom.createElement("div", [
    "flex", "flex-col", "md:flex-row", "justify-between", "items-center", "gap-4", "mb-4"
  ]);

  dom.appendToParent(controlsRow, [toggleViewBtn, inputWrapper]);
  ...
  let loadMoreBtn = dom.createElement("button", [
    "bg-red-700", "text-white", "font-medium",
    "px-6", "py-3", "rounded-lg", "mt-6", "cursor-pointer", "mx-auto"
  ], "Load More..");

  loadMoreBtn.onclick = () =>
    renderRecipes(allRecipes, cardsDiv, loadMoreBtn, sippenarDiv, false, shownCountObj);

  let sippenarDiv = dom.createElement("div", [
    "flex", "items-center", "justify-center", "mt-2", "mx-auto", "w-full", "bg-white", "z-50"
  ]);
  ...
  const fetchAll = () => {
    api.getRecipes((data) => {
      ...
      renderRecipes(allRecipes, cardsDiv, loadMoreBtn, sippenarDiv, true, shownCountObj);
    });
  };
 ...
 const createRecipeDetails = (recipe) => {
  ...
  let ingredientsTitle = dom.createElement("h2", [
  ...
  let instructionsTitle = dom.createElement("h2", [
  ...
  let goBackBtn = dom.createElement("button", [
    "bg-red-700", "text-white", "font-medium",
    "px-4", "py-2", "rounded-md", "hover:bg-red-800", "mt-6"
  ], "Go Back");
  ...
  dom.appendToParent(info, [
    title, ingredientsTitle, ingredientsList,
    instructionsTitle, instructionsList, goBackBtn
  ]);
